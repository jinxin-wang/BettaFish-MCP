...
├── start_crawl_data        # 启动 MindSpider 数据爬取 (异步)
├── get_crawl_data_status   # 查询爬取进度 (轮询)
├── get_crawl_data_result   # 获取爬取结果
├── subscribe_crawl_data    # 订阅爬取进度 (SSE)
├── cancel_crawl_data       # 取消爬取任务
├── start_crawl_topics      # 启动热点话题提取 (异步)
├── get_crawl_topics_status # 查询话题提取进度
├── get_crawl_topics_result # 获取话题提取结果
├── subscribe_crawl_topics  # 订阅 SSE 进度
├── cancel_crawl_topics     # 取消话题提取任务
├── start_crawl_social      # 启动社交媒体爬取 (异步)
├── get_crawl_social_status # 查询爬取进度
├── get_crawl_social_result # 获取爬取结果
├── subscribe_crawl_social  # 订阅 SSE 进度
└── cancel_crawl_social     # 取消爬取任务
...
│       ├── crawl.py                  # Level 1/2: crawl_data → start_crawl_data 异步
│       │                             # Level 2: start_crawl_topics / start_crawl_social
│       │                             # Level 2: check_spider_status (同步)
...
- **模式**: 同步
- **内部调用**: `MindSpider.check_config() / check_database_connection()`
- **返回**: 爬虫系统状态
...
#### start_crawl_data / get_crawl_data_status / get_crawl_data_result / subscribe_crawl_data / cancel_crawl_data
- **文件**: `mcp/tools/crawl.py`
- **模式**: 异步
- **提交参数**:
  - `keywords`: 关键词列表
...
#### start_crawl_topics / get_crawl_topics_status / get_crawl_topics_result / subscribe_crawl_topics / cancel_crawl_topics
- **文件**: `mcp/tools/crawl.py`
- **模式**: 异步
- **提交参数**:
...
#### start_crawl_social / get_crawl_social_status / get_crawl_social_result / subscribe_crawl_social / cancel_crawl_social
- **文件**: `mcp/tools/crawl.py`
- **模式**: 异步
- **提交参数**:
  - `platforms`: 平台列表
...
### 阶段二：Level 1 Tools (已完成，部分已升级为异步)
...
- [x] 2.6 实现爬虫状态检查工具（mcp/tools/crawl.py - check_spider_status）
...
### 阶段六：Level 2 Tools (异步模式) (已完成)

- [x] 6.1 实现 start_search_full / get_search_full_status / get_search_full_result / subscribe_search_full / cancel_search_full（mcp/tools/search.py）
- [x] 6.2 实现 start_media_full 系列（mcp/tools/media.py）
- [x] 6.3 实现 start_sentiment_full 系列（mcp/tools/sentiment.py）
- [x] 6.4 实现 start_crawl_data 系列（mcp/tools/crawl.py）
- [x] 6.5 实现 start_crawl_topics 系列（mcp/tools/crawl.py）
- [x] 6.6 实现 start_crawl_social 系列（mcp/tools/crawl.py）
- [x] 6.7 注册 Level 2 工具到 blueprint.py

### 阶段七：Level 3 Tools (异步模式) (已完成)

- [x] 7.1 创建 mcp/tools/forum.py
- [x] 7.2 实现 start_forum_research / get_forum_progress / get_forum_result
- [x] 7.3 实现 subscribe_forum SSE 订阅
- [x] 7.4 实现 stop_forum_research
- [x] 7.5 实现 get_forum_discussion
- [x] 7.6 扩展 start_report / get_report_status / get_report_result
- [x] 7.7 注册 Level 3 工具到 blueprint.py
